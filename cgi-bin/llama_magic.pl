...
my $DBLIST_VIEWFILE = "db_list_public.txt";
my $MSLIST_VIEWFILE = "ms_list_public.txt";
my $SHOW_SCORE = 0;
my $USE_PRIMERS = 0;
...
eval #for exception handling
{
	...
	{#form data was posted: process the data
		...
		else
		{
		    ...
		    my $proc1 = Proc::Background->new('C:/perl/bin/perl.exe', 'run_llama_scripts.pl', 'db_scripts', "$BASE_DIR/$RESULTS_DIR/$new_dir", $USE_PRIMERS);
		...
		else
		{
		    ...
		    my $proc1 = Proc::Background->new('perl.exe', 'run_llama_scripts.pl', 'search_and_map_scripts', "$BASE_DIR/$RESULTS_DIR/$db_id/$new_dir", "$BASE_DIR/$RESULTS_DIR/$db_id",
						      "$parent_err", "$frag_err", "$parent_err_units", "$frag_err_units", "/$RESULTS_DIR/$db_id/$new_dir", $SHOW_SCORE, $USE_PRIMERS);
...
sub display_home_page
{
	...
	if($db_lines[$i] =~ /^(\d+)\t([^\t]+)$/)
	{
		...
		#print a({href=>"../llama-magic-cgi/llama_magic.pl?submit=DBSelected;db_id=$id"}, "$name"), br();
	    }
...
sub display_ms_upload_form
{
	...
	'Fragment mass error:&nbsp;',
	textfield(-name=>'frag_mass_error', -value=>'0.4', -size=>4, -maxlength=>4),
	'<input type="radio" name="frag_error_units" value="Daltons" checked>Daltons',
	'<input type="radio" name="frag_error_units" value="ppm">ppm', br(), br(),
...
sub top_header
{
    ...
    print header(),
	  ...
	  print start_multipart_form(-method=>'POST', -action=>'../llama-magic-cgi/llama_magic.pl'),
	  ...
	  #a({href=>"../llama-magic-cgi/llama_magic.pl?submit=Home"}, "Home"),
	  #  "&nbsp;|&nbsp;",
